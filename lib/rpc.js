...
onNet("rpc:register", function (name) {
...
if (!isServer) {
	onNet("rpc:names", function (names) {
		for (const name of names) {
			log(`adding server rpc [${name}]`);
			registeredRPC[name] = true;
		}
	});
	emitNet("rpc:request-rpc-names");
}

if (isServer) {
	onNet("rpc:request-rpc-names", function () {
		const playerServerId = global.source;
		emitNet("rpc:names", playerServerId, Object.keys(localRegisteredRPCNames));
